...
日本語版を仕様の基準とします。パッケージとルートの英語入口は `README.en.md`、
英語チュートリアルは `docs/tutorials/en/<日本語と同じファイル名>` に置きます。
README は概要・実行ファイル・ビルド・実行手順を英語で提供し、00 は全章への英語索引、
01〜06 は基礎操作を英語で完結できるガイドとします。応用章は日本語リンクと明記します。

API・topic・parameter・コマンドを変更する PR では対応する日英ガイドも同時更新してください。
英語未対応の新章は 00 の索引に Japanese と明記し、翻訳待ちとして追跡します。
`python3 scripts/check_docs_consistency.py` で入口・実行名・相対リンクを検証します。
本文の意味の一致は機械検証できないため、レビュー時に日本語版と照合します。

統合テストはビルド後 `bash scripts/test-integration.sh` で実行します。
ROS / launch_testing がない環境での静的検証を、ノード間統合テストの成功とは扱いません。
